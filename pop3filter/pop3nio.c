...
static unsigned connection_done(struct selector_key *key)
{
    ...
    if ((error = getsockopt(key->fd, SOL_SOCKET, SO_ERROR, &error, &len)) < 0)
    {
        ...
        if (SELECTOR_SUCCESS == selector_set_interest(key->s, proxy_pop3->client_fd, OP_WRITE))
        { // Setear cliente para escritura
...
static void filter_read(struct selector_key *key)
{
    ...
    if (n == 0 || n == -1)
    {
        parse_headers(c, ATTACHMENT(key)->state_filter_in);
...
static void filter_write(struct selector_key *key)
{
    ...
    //int index = parse_headers(key);

    //strcpy(headers_buffer,ptr,index);
    //buffer_read_adv(index);
